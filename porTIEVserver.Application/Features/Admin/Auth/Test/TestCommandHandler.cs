...
namespace porTIEVserver.Application.Features.Admin.Auth.Test
{
    internal sealed class TestCommandHandler(
        ...
        IJwtProvider jwtProvider) : IRequestHandler<TestCommand, Result<LoginCommandResponse>>
    {
        public async Task<Result<LoginCommandResponse>> Handle(TestCommand request, CancellationToken cancellationToken)
        {
            AppUser appUser = await userManager.Users
                .FirstOrDefaultAsync(p =>
                p.UserName == Parameters.TestUser_UserName,
                cancellationToken);
            ...
            SignInResult signInResult = await signInManager.CheckPasswordSignInAsync(appUser, Parameters.TestUser_Password, true);
